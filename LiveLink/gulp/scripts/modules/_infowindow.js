var infowindow = {
	init: function() {
		$(".infowindow:not(.js-infowindow-done)").each(function() {
			...
			$(this).find(".carousel-control").off("click").on("click",
				function(e) {
			...
			$(this).find(".js-watch:not(.js-watch-done)").each(function() {
				...
				$(this).click(function() {
					...
					userservice.watch(id,
						this,
						function(response, button) {
							if (response.Success) {
								if (response.Data.Active) {
									$(button).addClass("active");
									notification.show("<i class=\"fa fa-eye\"></i> Now watching <b>" +
										response.Data.Title +
										"</b>",
										"success");
								} else {
									$(button).removeClass("active");
									notification.show("<i class=\"fa fa-eye\"></i> Stopped watching <b>" +
										response.Data.Title +
										"</b>",
										"warning");
								}
